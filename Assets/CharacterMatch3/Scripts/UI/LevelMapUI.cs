...
namespace CharacterMatch3.UI
{
    public sealed class LevelMapUI : MonoBehaviour
    {
        ...
        private const int SegmentThemeCount = 8;
        ...
        private static readonly string[] SegmentThemeNames =
        {
            "Forest",
            "Beach",
            "Desert",
        ...
        private static readonly Vector2[][] RoadLevelPositionFactors =
        {
            new[]
            {
        ...
        [SerializeField, Range(1f, 1.35f)] private float mapArtZoom = 1f;
        ...
        private void Build()
        {
            ...
            var highestLevelNumber = levelLibrary != null ? levelLibrary.HighestLevelNumber() : 0;
            maxLevelNumber = Mathf.Clamp(
                highestLevelNumber > 0 ? highestLevelNumber : CharacterMatch3Constants.LastLevel,
                CharacterMatch3Constants.FirstLevel,
                CharacterMatch3Constants.LastLevel);
        ...
        private bool TryUseSceneEditableMapViewport()
        {
            ...
            if (CountSceneEditableLevelNodes(contentRoot) < maxLevelNumber)
            {
                sceneMapPreviewRoot.gameObject.SetActive(false);
                contentRoot = null;
                scrollRect = null;
        ...
        private static int CountSceneEditableLevelNodes(Transform root)
        {
            var count = 0;
            if (root == null)
            {
            ...
            {
                if (FindDeepChildByName(root, $"PreviewLevel_{levelNumber:000}") != null)
                {
                    count++;
                }
            }
        ...
        {
            var sections = Mathf.Max(1, mapSectionCount);
            var clamped = Mathf.Clamp(segmentFromBottom, 0, sections - 1);
            var height = 1f / sections;
            return new Rect(0f, clamped * height, 1f, height);
        }
        ...
        {
            var index = GetSegmentThemeIndex(segmentFromBottom);
            return index >= 0 && index < SegmentThemeNames.Length ? SegmentThemeNames[index] : "Map";
        }
        ...
        private void CreateLevelNodes()
        {
            ...
            for (var levelNumber = CharacterMatch3Constants.FirstLevel; levelNumber <= maxLevelNumber; levelNumber++)
            {
                ...
                UIFactory.SetAnchored(starsLabel.rectTransform, new Vector2(-0.2f, -0.38f), new Vector2(1.2f, -0.04f), Vector2.zero, Vector2.zero);
        ...
        {
            var themeIndex = GetSegmentThemeIndex(segmentFromBottom);
            if (themeIndex >= 0 && themeIndex < RoadLevelPositionFactors.Length)
            {
                return RoadLevelPositionFactors[themeIndex];
            }
